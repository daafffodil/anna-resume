...
• 平台策略输出：为字节跳动（巨量引擎、抖音生活服务）、小红书提供商业化策略支持，主导编写《小红书线索经营通案》、《千帆推广通案》等官方标准课件
• 全球营销洞察：负责小红书全球时装周营销调研，通过对奢品/时尚行业的深度扫描，产出行业峰会核心策略内容
...
学术背景为后续从事品牌运营、营销策略、产品运营等工作奠定了坚实的理论与实践基础`,
  ...
  const handleShareResume = (resume) => {
    ...
    if (navigator.share) {
      ...
      const url = `${window.location.href}?resume=${resume.id}`;
      alert(`分享链接：${url}\n\n${shareText}`);
    }
  };
